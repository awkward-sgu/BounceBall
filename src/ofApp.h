...
class ofApp : public ofBaseApp{
	...
	public:
		...
		void loadMap(short int level, short int difficulty); // load map based on level and difficulty
		...
		short int menuFlag; // set to 1 initially // 1. menu screen // 2. choose difficulty screen // 0. play screen
		short int loadingTime; // shows loading screen when loadingTime is true
		short int currentLevel;
		short int difficulty;
		short int mouseBuffer; // to prevent overclick
		short int helpFlag; // tutorial help

		float xScale; // scale of x : based on screen size
		float yScale; // scale of y : based on screen size

		bool full; // full screen on/off
		short int screenBuffer; // to prevent overclick : full screen on/off
